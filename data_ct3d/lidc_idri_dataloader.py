...
#parameters
PATH_TO_DICOM_INFO = '/data/pycode/LungCT3D/data_ct3d/LIDC_DICOM_info.txt'
PATH_TO_TRAIN_FILE = '/data/pycode/LungCT3D/data_ct3d/LIDC_VR_Train.txt'
PATH_TO_TEST_FILE = '/data/pycode/LungCT3D/data_ct3d/LIDC_VR_Test.txt'
...
class DatasetGenerator(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for img, mask in zip(vol_imgs, vol_masks):
            ...
            img = zoom(img, (VOL_DIMS[1]/img.shape[0], VOL_DIMS[2]/img.shape[1]), order=1)
            img = torch.as_tensor(img, dtype=torch.float32)
            ts_imgs = torch.cat((ts_imgs, img.unsqueeze(0)), 0)
            
            mask = zoom(mask, (VOL_DIMS[1]/mask.shape[0], VOL_DIMS[2]/mask.shape[1]), order=1)
            mask = torch.as_tensor(mask, dtype=torch.float32)
            ts_masks = torch.cat((ts_masks, mask.unsqueeze(0)), 0)
        ts_imgs = zoom(ts_imgs, (VOL_DIMS[0]/ts_imgs.shape[0], 1, 1), order=1)
        ts_imgs = torch.as_tensor(ts_imgs, dtype=torch.float32)
        ts_imgs = ts_imgs.unsqueeze(0) #CxDXHxW
        ts_masks = zoom(ts_masks, (VOL_DIMS[0]/ts_masks.shape[0], 1, 1), order=1)
        ...
        return ts_imgs, ts_masks, ts_nodvol
...
if __name__ == "__main__":
    ...
    #for debug   
    datasets = get_train_dataloader(batch_size=8, shuffle=True, num_workers=1)
    for batch_idx, (ts_imgs, ts_masks, ts_nodvol) in enumerate(datasets):
        print(ts_imgs.shape)
